...
| Load files              | No direct `load()` in this function, but it depended on downstream implicit loads and opened DB internally                  | No strict-path internal `load()` and no internal `db.open()`; all required maps, including TRY-derived `trait_data_by_pft`, are passed explicitly                                  |
...
| Flow                    | Opened a BETY connection internally, looped over PFTs, and relied on downstream implicit loads in `run.meta.analysis.pft()` | Orchestrates per-PFT execution from explicit TRY-derived trait-data maps without internal `load()` or `db.open()` in the strict path                                                |
...
| `workflow.bm.R`               | `scripts/workflow.bm.R`                                         |
| `workflow.pda.R`              | `scripts/workflow.pda.R:131`                                    |
| `interactive-workflow.R`      | `tests/interactive-workflow.R`                                  |
| `test.run.meta.analysis.R`    | `modules/meta.analysis/tests/testthat/test.run.meta.analysis.R` |
...
| Load files              | Loaded `trait.data.Rdata` and `prior.distns.Rdata` from each `pft$outdir`                                                                                | No strict-path file load; required TRY-derived `trait_data` and `priors` are passed explicitly                                                      |
...
| Flow                    | Loaded `trait.data.Rdata` and `prior.distns.Rdata`, ran `meta_analysis_standalone()`, saved workflow outputs, then copied/registered files in DB storage | Computes from explicit TRY-based trait-data objects, optionally writes workflow outputs, and optionally registers them in DB storage                |
...
| Caller function            | Location                                                           |
| -------------------------- | ------------------------------------------------------------------ |
| `run.meta.analysis`        | `modules/meta.analysis/R/run.meta.analysis.R:324`                  |
| `test.run.meta.analysis.R` | `modules/meta.analysis/tests/testthat/test.run.meta.analysis.R:21` |
|                            |                                                                    |
...
| Load files              | Relied on downstream hidden file loads from `pft$outdir` when object maps were absent | No strict-path file load; explicit TRY-derived trait-data maps are passed in, and fallback loading is isolated in the wrapper                           |
...
| Flow                    | Delegated to `run.meta.analysis(...)` with hidden file and DB behavior below it       | Wrapper extracts only required attrs, supports `Settings` and `MultiSettings` orchestration, and uses a deprecated fallback loader only when explicit TRY-derived object maps are missing |
...
```text
Caller
  -> prepare explicit maps: TRY-derived trait_data_by_pft, prior_distns_by_pft, optional dbcon
