...
#' the config-writing machinery. It expects a pre-generated input design bundle
#' (including samples), then delegates to \code{\link{run.write.configs}} for
#' actual config generation. For MultiSettings, pass one shared design bundle
#' generated from \code{\link{generate_input_design}} to keep sampling consistent
#' across all sites.
...
#' \enumerate{
#'   \item Validate that \code{input_design} includes explicit designs plus
#'         \code{samples}.
...
#'   \code{\link{generate_input_design}}. Must be a list with
#'   \code{samples} and optional \code{ensemble}/\code{sensitivity}
#'   data.frames.
...
runModule.run.write.configs <- function(settings,
  ...
  if (is.null(input_design) || !is.list(input_design)) {
    stop(
      ...
      "Generate input design first via generate_input_design(settings, dbCon) ",
  ...
  if (is.null(input_design$samples)) {
    stop(
      "runModule.run.write.configs requires `input_design$samples`. ",
      "Generate input design first via generate_input_design(settings, dbCon) ",
      "and pass it to runModule.run.write.configs(..., input_design = ...)."
    )
  }
...
#' Generate explicit input designs and samples for config writing
#'
#' Generates normalized design matrices and structured samples object to be
#' passed into \code{\link{runModule.run.write.configs}}.
#'
#' @param settings a PEcAn Settings or MultiSettings object
...
#'   entries.
#' @param dbCon a valid, open DB connection.
#' @return A list with \code{ensemble}, \code{sensitivity}, and \code{samples}.
#' @importFrom rlang %||%
#' @export
generate_input_design <- function(settings, input_design = NULL, dbCon = NULL) {
  if (is.null(dbCon)) {
    stop(
      "generate_input_design requires a non-NULL `dbCon`.",
      " Open the DB connection before generating input designs."
    )
  }
  ...
  if (!is.null(base_settings$ensemble) &&
      is.null(base_settings$ensemble$samplingspace$parameters$method)) {
    base_settings$ensemble$samplingspace$parameters$method <- "uniform"
  }
