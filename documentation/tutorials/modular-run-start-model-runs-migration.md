...
1. Preferred path: caller prepares and passes required run inputs explicitly:
  ...
  - optional write control: `write_files` (defaults to `TRUE`)
2. `runModule_start_model_runs(settings, runs, dbCon = NULL, write_files = TRUE, ...)` keeps
  `settings` as a required input for orchestration and Settings vs
   MultiSettings dispatch.
3. `start_model_runs(runs, rundir, modeloutdir, host, model_type, dbCon = NULL, write_files = TRUE, ...)`
...
| Aspect              | Legacy Pattern                                                  | New Pattern                                                                                                   |
| ------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Main API            | `start_model_runs(settings, write = TRUE, ...)`                 | `start_model_runs(runs, rundir, modeloutdir, host, model_type = NULL, dbCon = NULL, write_files = TRUE, ...)` |
...
| Return             | Open file connection                                                               | Open file connection                                                          |
...
| Output               | Submission response only                                     | Submission response only                                                          |
...
1. `base/workflow/tests/testthat/test-runModule-start-model-runs.R`
